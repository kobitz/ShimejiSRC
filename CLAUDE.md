...
- `FanToggleScanCode` (default 61) — Hardware scan code for the fan-max toggle key. `FanController` spawns `fan_toggle.py` (must be next to JAR) with this value when CampfireON_blue activates/deactivates or CPU temp crosses 75 deg C. Scan code is used because the key reports VK 255 (not a valid Java VK), so `Robot` cannot send it. Run `fan_key_detect.py` to find the scan code if yours differs.
...
| `src/.../environment/FanController.java` | Singleton that toggles fan-max by spawning fan_toggle.py with a scan code. Triggered by Manager (CampfireON_blue active) and CpuTempMonitor (temp drops below 75 deg C). |
...
- **Paimon third-person adherence** — `<SpeechRule>` + third-person personality + few-shot examples get llama3.1:8b most of the way, but it still occasionally slips into "I"/"me". Remaining lever is a post-generation regex rewrite (no bigger model feasible on this hardware).
