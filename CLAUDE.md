...
- **`OllamaClient.java`** — HTTP client. Queue depth 5, 2s floor, drops oldest on full. **`keep_alive` must be a duration string ("10s")** — bare JSON numbers are parsed by Go's `time.Duration` as nanoseconds, so the old `keep_alive:10` meant 10ns = unload instantly. Split keep-alive: text via `TEXT_KEEP_ALIVE_SEC` (currently 120 — covers the 45-90s cadence; for partly-CPU-resident models that holds their RAM share warm), vision 0s — **except when VisionModel == OllamaModel** (shared multimodal model): vision calls then use the text linger so a screen glance doesn't evict the warm text model. **`num_gpu`: -1 (Ollama auto-fit) when GPU ≥50% idle** — old `0.20` layer cap forced ~80% of every model into system RAM and cut gemma3:4b from 82 to 15 tok/s; busy GPU scales layers by available fraction. The cap was also the cause of long-standing system-wide CPU spikes (every reaction ran mostly on CPU — which could even self-trigger CPU-reactive behaviors like 2B's GlitchingOut and Campfire scaling). Sends `"think":false` — required for Gemma 4/thinking models (without it they return empty `response`). 90s timeout for cold loads. Model fit on 6 GB RTX 3060: gemma3:4b = 100% GPU; gemma4:e4b = 69% CPU/31% GPU (~6.5 GB system RAM while loaded, inherent to its 9.5 GB size).
...
- **`MascotSpeechRegistry.java`** — Inter-mascot speech. `WeakReference` listeners — callers keep strong ref. `pendingPairs` prevents duplicate reactions per speaker-listener pair. Chain depth capped at 2 (original → reaction → counter-reaction, then stop); reaction probability `{0.35, 0.2}` per depth. Longer chains degenerate into content-free meta-commentary spirals.
...
- **`QUICK_REACTION_STYLE_RULES`** — Shared rules constant injected into all 4 unprompted-reaction prompts (peer, spontaneous, audio, vision): REACT-don't-report (must contain an opinion/feeling/quip about ONE detail — bans caption-style description and "X while Y" context-splicing), never grade anyone's statement, no "appears"/"seems" hedging, context lines are background-only. Evolved June 2026 across two chat.log audits: banning meta-grading alone made neutral *description* the model's fallback ("You are viewing a Nintendo Direct right now") — rules must demand a take, not just concreteness.
...
- **Memory poisoning** — peerExchanges reinforce model patterns quickly. Wipe when personality degrades. Partially mitigated June 2026: observed facts no longer store reaction text; peer chains capped at depth 2.
