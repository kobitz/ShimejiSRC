...
- **`Manager`**: Tick loop. `WindowsEnvironment.beginTick()` runs once per tick (not per mascot). **Tick watchdog (June 2026):** any tick ≥120ms logs `[TickWatch] Slow tick:` WARNING with phase breakdown — envScan ms, slowest mascot (image set + ms), mascot count, heap used/total, cpu/gpu load. Unattributed time (total − envScan − mascots) = lock wait (EDT holding the mascots lock) or GC. Added to diagnose intermittent ~1s drops to 3-4 fps; check ShimejieeLog for these lines after a stutter.
