...
- **`Manager`**: Tick loop. `WindowsEnvironment.beginTick()` runs once per tick (not per mascot). **Tick watchdog (June 2026):** any tick ≥200ms (originally 120; raised once the starvation fixes landed) logs `[TickWatch] Slow tick:` WARNING with phase breakdown — envScan ms, slowest mascot (image set + ms), mascot count, heap used/total, cpu/gpu load. Unattributed time (total − envScan − mascots) = lock wait (EDT holding the mascots lock) or GC. Added to diagnose intermittent ~1s drops to 3-4 fps; check ShimejieeLog for these lines after a stutter.
...
| `TempSensor.exe` | CPU temp streamer (next to JAR, requires admin). Built from `SensorReader/Program.cs` via `dotnet publish`. Also toggles MSI Cooler Boost (EC 0x98/0x80 via `MSI_ACPI` `Get_Data`/`Set_Data` WMI, read-modify-write) on `fan_on`/`fan_off` stdin. |
...
- **MSI Cooler Boost (implemented June 2026)** — Driven by the `CoolerBoost` affordance: any mascot broadcasting it (any Action carrying `Affordance="CoolerBoost"`) ramps both fans to max; none broadcasting turns it off. `Manager.checkCoolerBoostState()` checks every tick (affordances toggle without a mascot-list change, so it cannot be gated on `listChanged`); `FanController` is level-triggered with a 7s re-assert and, per toggle, **spawns a fresh short-lived `TempSensor.exe boost on|off` process** (new WMI connection, dies immediately) that writes EC register **0x98** bit **0x80** via the `MSI_ACPI` WMI interface — the same firmware path MSI Center uses. The long-running TempSensor.exe is now purely the temperature streamer; fan commands do NOT go through its stdin (that pipe was a failure source — see below). **FIXED June 13 2026; validated 1 hr / 405-of-405 toggles.** Needs admin; TempSensor is already elevated. Key mechanics learned the hard way:
  ...
  - **Dispose the WMI objects.** Every `ManagementScope`/`Searcher`/`ManagementObject`/`ManagementBaseObject`/`ManagementClass` is an IDisposable COM wrapper. The first version leaked them on every call; over a long session (campfire oscillating blue↔orange = many toggles) WMI ran out of handles and `Set_Data` silently failed — Cooler Boost **worked fresh, died after a few minutes, and a restart fixed it** (temps kept working because those use LHM, not WMI). Fix (June 2026, commit ffaa44c): cache the `MSI_ACPI` instance (connect once), dispose every per-call object, drop the cache on error. **This stopped the handle leak but its "connect once" caching then CAUSED the next bug — see RESOLVED below.**
  - **RESOLVED (June 13 2026): the ~10-min in-app stall was a hung WMI/COM call freezing the whole process.** Root cause: the cached long-lived `MSI_ACPI` connection eventually **hangs on a call** (~15 min / N toggles in). A blocking WMI/COM call sits in unmanaged code and never hits a GC safe-point → the .NET GC can't run → **every managed thread freezes**, so the in-process 8 s watchdog couldn't even fire (`tempsensor.log` showed `received: fan_off` with no completion and no `WMI HANG` line; process alive, `responding=True`, ~5 s CPU over 30 min — wedged, not crashed). Diagnosed entirely from `tempsensor.log` after adding per-command `received:` logging. **Fix: process isolation.** The long-running TempSensor no longer touches WMI; each `fan_on`/`fan_off` spawns a SHORT-LIVED CHILD (`TempSensor.exe boost on|off`, the CLI mode) that opens a *fresh* WMI connection, does the one EC write, and exits — so no long-lived COM state accumulates to go stale. That child-spawned-by-TempSensor version ran 40 min / 82-of-82 but then revealed a DEEPER bug: the **long-lived Java→TempSensor stdin pipe silently stops delivering after ~20 min** — `fancontroller.log` showed the Java side sending fine (no `checkError`), while `tempsensor.log` froze with TempSensor alive but its `ReadLine` never returning (no EOF, no respawn). **Final fix (commit pending): retire the stdin command pipe entirely.** `FanController` (Java) now spawns `TempSensor.exe boost on|off` directly per toggle — fresh WMI connection, dies immediately — so there is no long-lived pipe to die and no long-lived WMI to freeze. TempSensor's stdin-reader + its own child-spawn path are now dead code (harmless; `boost on|off` CLI mode is what gets used). Validated: **1 hr, 405 spawns / 405 completions, 0 failures.** **Lessons:** (1) in-process WMI can't be made watchdog-safe (a COM hang freezes the GC → all threads); isolate killable work in a separate process. (2) long-lived stdin pipes to a child process are fragile — prefer spawning a short-lived process per command. (3) "works fresh, dies after a while" cycled through FOUR wrong/incomplete root-causes (external reset → handle leak → process freeze → stdin pipe) before two-sided logging nailed it — instrument both ends and read the logs, don't theorize, and "worked for N minutes" is never proof until it's well past the failure point.
  - TempSensor CLI test modes (run elevated): `boost on|off` (RMW + self-verifying read-back), `read <hexReg>`, `write <hexReg> <hexVal>`, `dump`.

  The old "EC blocks all software paths" belief was wrong: TempSensor's fan handler had always been a literal no-op (it never wrote the EC), and the real symptom was an unplugged fan 1 (reseated June 2026 — MSI Center's software Cooler Boost then worked, proving the EC accepts writes). CampfireON_blue broadcasts the affordance from its `Stand` + `ScaleWithLoad` actions. The hardware Fn+F8 button still works independently. (Blue fire is often active only 3-5 s at a time, so boost may pulse briefly — tune blue-fire lifetime if that's undesirable.)
...
- **AMD compatibility (planned — Ko getting an MSI AMD laptop)** — Core app, Whisper (CPU int8), and Ollama (ROCm/CPU fallback, num_gpu=-1) already work on AMD.
  - **(1) CPU temp — DONE (June 2026):** `SensorReader/Program.cs` `ReadCpuTemp()` now picks by a vendor fallback rank — `Core Average` (Intel) → `Core (Tctl/Tdie)` (Ryzen) → any Tctl/Tdie variant → `CPU Package` → any CPU-node temperature sensor — and logs the chosen sensor name once to stderr (`CPU temp sensor: "..."`). Additive: Intel still hits `Core Average` first (rank 0), so zero regression. Landed blind on purpose — it can only degrade gracefully, never break the Intel/NVIDIA path. On the Ryzen machine, confirm the stderr log names a real sensor (not -1).
