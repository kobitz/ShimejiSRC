...
- **MSI Cooler Boost (implemented June 2026)** — Driven by the `CoolerBoost` affordance: any mascot broadcasting it (any Action carrying `Affordance="CoolerBoost"`) ramps both fans to max; none broadcasting turns it off. `Manager.checkCoolerBoostState()` checks every tick (affordances toggle without a mascot-list change, so it cannot be gated on `listChanged`); `FanController` de-dupes (one WMI call per real transition) and pipes `fan_on`/`fan_off` to TempSensor.exe, which writes EC register **0x98** bit **0x80** via the `MSI_ACPI` WMI interface — the same firmware path MSI Center uses. Verified working via the CLI (`TempSensor.exe boost on` → fans ramp); **the in-app long-running path has an OPEN BUG — see below.** Needs admin; TempSensor is already elevated. Key mechanics learned the hard way:
  ...
  - **Dispose the WMI objects.** Every `ManagementScope`/`Searcher`/`ManagementObject`/`ManagementBaseObject`/`ManagementClass` is an IDisposable COM wrapper. The first version leaked them on every call; over a long session (campfire oscillating blue↔orange = many toggles) WMI ran out of handles and `Set_Data` silently failed — Cooler Boost **worked fresh, died after a few minutes, and a restart fixed it** (temps kept working because those use LHM, not WMI). Fix (June 2026, commit ffaa44c): cache the `MSI_ACPI` instance (connect once), dispose every per-call object, drop the cache on error. **This was a real bug but did NOT resolve the field symptom — see OPEN BUG.**
  - **OPEN BUG (June 13 2026, UNRESOLVED): Cooler Boost stops after ~10 min of runtime.** Fresh launch works; ~10 min in, the blue campfire stops triggering boost; it resumes *without* a full Shimeji restart when the TempSensor *child* respawns. Revised hypothesis (the WMI leak was NOT it): **the `TempSensor.exe` process itself is dying** around 10 min, and `CpuTempMonitor`'s auto-respawn (re-spawns the on-disk exe ~5 s after exit) is what revives it. Prime suspect: an intermittent LHM `hw.Update()` exception in the temp loop propagating out of `Main` and crashing the process (the loop had no try/catch). Secondary: the stdin-reader's EOF/exception path silently `Environment.Exit`ing. **Instrumentation added + deployed, NOT yet root-caused** (uncommitted-then-committed this session): TempSensor now writes `tempsensor.log` next to the exe (install folder) logging each (re)start with PID, fan commands + EC read/write results, temp-read errors, and the exit reason (stdin EOF / stdin exception / global `UnhandledException` handler); the temp loop now catches+continues so a transient LHM throw can't kill it. **NEXT STEP for future-me:** relaunch elevated, reproduce the ~10-min stall (or confirm the hardening killed it), then read `tempsensor.log`. Multiple `===== TempSensor started =====` lines ⇒ it's dying/respawning and the line just above each restart names *why*; one start line with boost commands still flowing ⇒ not dying, bug is in delivery/EC. **Discipline:** a fresh process always works briefly, so "it worked after a change" is NOT proof — only surviving past ~10 min is. (Two confident root-causes were already wrong this session — external-reset, then the WMI leak — so verify via the log before declaring it fixed.)
