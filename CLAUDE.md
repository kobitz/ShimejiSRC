...
- **`firePeerReaction()`** — Triggered by `MascotSpeechRegistry`. Uses `buildLightMemoryBlock()`. Filtered by `isTrivialAcknowledgement()` — responses that normalize to "noted", "acknowledged", "understood", "affirmative", "confirmed", "copy that", "roger", or "indeed" are silently dismissed without recording.
