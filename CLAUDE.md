...
- **AMD compatibility (planned — Ko intends to get an AMD machine)** — Core app, Whisper (CPU int8), and Ollama (ROCm/CPU fallback, num_gpu=-1) already work on AMD. Two telemetry gaps: (1) BUG: `SensorReader/Program.cs` matches the LHM sensor named exactly "Core Average" (Intel-only); Ryzen exposes "Core (Tctl/Tdie)" → cpuTemp stays -1 → CampfireOFF ignition never fires. Fix: fallback chain (Core Average → Tctl/Tdie → any CPU Temperature sensor). (2) gpuTemp/Load/Mem come only from nvidia-smi → -1 on AMD, GPU-driven ambient features inert (degrade gracefully, no crashes). Fix: extend TempSensor.exe with `IsGpuEnabled = true` (LHM exposes AMD GPU core load/temp/VRAM) streaming `gpuLoad=` etc. lines; `CpuTempMonitor` uses them as fallback when nvidia-smi absent. Trade-off: AMD GPU stats then need TempSensor's admin elevation. Write LHM sensor-name matching defensively (naming varies by GPU generation) and verify on the target machine — do NOT ship blind.
